...
from registration import icp, match_ransac
...
def marker_registration(source,target):
     ...
     #gray_des = cv2.cvtColor(cad_des.astype(np.uint8), cv2.COLOR_RGB2GRAY)
     aruco_dict = aruco.getPredefinedDictionary(aruco.DICT_4X4_50) # Aqui era 250
     parameters = aruco.DetectorParameters()
     parameters.adaptiveThreshConstant = 6
     parameters.perspectiveRemovePixelPerCell = 200
    
     #lists of ids and the corners beloning to each id
     corners_src, _ids_src, rejectedImgPoints = aruco.detectMarkers(cad_src, aruco_dict, parameters=parameters)
     corners_des, _ids_des, rejectedImgPoints = aruco.detectMarkers(cad_des, aruco_dict, parameters=parameters)

     
     try:
         ids_src = [_ids_src[i][0] for i in range(len(_ids_src))]
         ids_des = [_ids_des[i][0] for i in range(len(_ids_des))]
     except:
         return None
     ...
     src_good = [
            depth_src[int(corner[1])][int(corner[0])]
            for i, id in enumerate(ids_des)
            if id in ids_src
            for count, corner in enumerate(corners_src[ids_src.index(id)][0])
            if depth_src[int(corner[1])][int(corner[0])][2] != 0 and depth_des[int(corners_des[i][0][count][1])][int(corners_des[i][0][count][0])][2] != 0
        ]

     dst_good = [
            depth_des[int(corners_des[i][0][count][1])][int(corners_des[i][0][count][0])]
            for i, id in enumerate(ids_des)
            if id in ids_src
            for count, corner in enumerate(corners_src[ids_src.index(id)][0])
            if depth_src[int(corner[1])][int(corner[0])][2] != 0 and depth_des[int(corners_des[i][0][count][1])][int(corners_des[i][0][count][0])][2] != 0
...
def full_registration(path,max_correspondence_distance_coarse,
     ...
     data = [load_images(path, source_id) for source_id in trange(n_pcds)]

     for source_id in trange(n_pcds):
          if source_id > 0:
               pcds[source_id-1] = []
          
          color_src = data[source_id][0] 
          depth_src = data[source_id][1]
          ...
          for target_id in range(source_id + 1, n_pcds, max(1,int(n_pcds/N_Neighbours))):
               ...
               color_dst = data[target_id][0] 
               depth_dst = data[target_id][1]
...
def load_pcds(path, downsample = True, interval = 1):
    ...
    for Filename in range(len(glob.glob1(path+"images","*.jpg"))/interval):
